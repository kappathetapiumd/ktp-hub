...
const weeks = [
  ...
  "4/6 - 4/12", "4/13 - 4/19", "4/20 - 4", "4/27 - 5/3", "5/4 - 5/8"
];

export default function Weeks() {
  const [activeWeek, setActiveWeek] = useState(getCurrentWeek(dayjs()));
...
function getCurrentWeek(today: dayjs.Dayjs) {
  for (let i = weeks.length - 1; i >= 0; i--) {
    const [start] = weeks[i].split(' - ');
    const startOfWeek = dayjs(start);

    if (today.isAfter(startOfWeek) || today.isSame(startOfWeek, 'day'))
      return weeks[i];
  }
